...
import { isOpenAiConfigured, openai } from "../client";
...
function requireOpenai() {
  if (!isOpenAiConfigured() || !openai) {
    throw new Error(
      "OpenAI is not configured. Set AI_INTEGRATIONS_OPENAI_API_KEY and AI_INTEGRATIONS_OPENAI_BASE_URL.",
    );
  }
  return openai;
}
...
export async function speechToText(
  ...
  const response = await requireOpenai().audio.transcriptions.create({
    file,
...
export async function speechToTextStream(
  ...
  const file = await toFile(audioBuffer, `audio.${format}`);
  const stream = await requireOpenai().audio.transcriptions.create({
    file,
    model: "gpt-4o-mini-transcribe",
    stream: true,
  });
